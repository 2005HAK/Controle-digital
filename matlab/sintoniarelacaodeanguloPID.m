...
% Zero 2: condição de angulo em s*
phi_p1 = angle(s_star - 0);
phi_p2 = angle(s_star - 0);

% Angulo que z2 precisa contribuir
phi_z2_needed = pi + phi_p1 + phi_p2;
z2 = real(s_star) - imag(s_star) / tan(phi_z2_needed);
fprintf('z2 pela condição de angulo = %.4f\n', z2);

% C(s) com os zeros calculados
% C(s) = Kd*(s-z1)*(s-z2)/s
num_zeros = conv([1 -z1], [1 -z2]);

% Ganho K pela condição de módulo em s*
L_s = (polyval(num_zeros, s_star) / s_star) * (0.0526 / (s_star^2 + 0.8842*s_star));
K = 1 / abs(L_s);
fprintf('K mínimo = %.4f\n', K);
...
sigma_min = sigma;
zeta_ang  = acos(zeta);
patch([ax(1) sigma_min sigma_min ax(1)], [ax(3) ax(3) ax(4) ax(4)], 'g', 'FaceAlpha', 0.1, 'EdgeColor', 'none');
lx = abs(ax(1));
patch([0 -lx -lx], [0 lx*tan(zeta_ang) -lx*tan(zeta_ang)], 'b', 'FaceAlpha', 0.1, 'EdgeColor', 'none');
...
title('Root Locus com zeros pela condicao de angulo');
